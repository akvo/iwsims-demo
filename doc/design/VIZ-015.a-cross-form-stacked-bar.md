...
**Author**: Iwan
**Date**: 2026-09-08
**Status**: Implemented, in the VIZ-015 PR (#349). 517 backend and 360
frontend tests pass; flake8, eslint and prettier clean. Sections 7 and 10
were rewritten after the build to match the shipped code rather than the
...
- [x] A bar chart's "Stack by" offers option questions from the other forms
...
- [x] The chart states that it counts sites, so a reader does not mistake
      it for a submission count — as a hint under the pinned Group by
      control, not as a caption on the chart itself
...
unwrapped on change by `stackChangeOf` and never stored nor sent.
`stackValueOf` reads the config back the other way, and the pair is
exported so the mapping is testable without rendering the inspector.
...
**Forty-one tests across seven files**, listed as they exist rather than
as they were planned. The ones marked **guard** protect an invariant that
...
### `pages/dashboards/__test__/BuilderInspector.test.js` — the control (12)
...
**Two things decide whether the entries appear at all**, and getting
either wrong makes the feature look absent rather than unavailable:

1. **Group by must be Registration site.** The join keys on the
   registration datapoint, which is only a key under that grouping.
2. **The Question must be single-choice.** A cross-form stack takes one
   category answer per site. Pick a multi-choice question and the
   other-form entries are withheld — the inspector says so beneath the
   control, but it is easy to hit by accident. Note that the stacking
   question *may* be multi-choice; only the measured one may not.
...
| Group by | Registration site |
| Stack by | under the group named after the registration form → e.g. "Who is the project Implementing agency/agencies?" (multi-choice is fine here) |
| Orientation | Horizontal |

| # | Steps | Expect |
|---|---|---|
| M-1 | Build the widget above, stopping before Stack by | "Stack by" lists None / Option value, then a group per **other** form in the family, each headed by the form's name |
| M-2 | Pick a question under one of those groups | Group by locks to Registration site and greys out, with a hint saying the chart counts registration sites. The chart redraws with the other form's options as its legend |
...
| M-4 | Change Question to a **multi-choice** question | The other-form groups disappear and a hint explains that a single-choice Question is needed. Any cross-form selection is cleared rather than left unsaveable |
| M-5 | Change Group by away from Registration site | The cross-form stack is cleared automatically by `withValidStack`, and the other-form groups are gone |
